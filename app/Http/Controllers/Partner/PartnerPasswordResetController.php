...
class PartnerPasswordResetController extends Controller
{
    ...
    public function sendResetLink(Request $request)
    {
        ...
        // メール送信
        $resetUrl = url('/partner/password-reset/' . $token . '?email=' . urlencode($request->email));
        Mail::to($request->email)->send(new PartnerPasswordResetMail($resetUrl, $admin->name));
    ...
    {
        return view('partner.password-reset-new', [
            'token' => $token,
            'email' => $request->email,
        ]);
    }
