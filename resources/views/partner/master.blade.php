...
<td class="mono" style="font-size:12px;">{{ ($device->current_pin && $device->current_pin !== $device->initial_pin) ? $device->current_pin : '-' }}</td>
                        ...
                        <td style="font-size:12px;{{ $device->status === 'alert' ? 'color:#c62828;font-weight:600;' : '' }}">{{ $device->last_human_detected_at ? $device->last_human_detected_at->format('m/d H:i') : '-' }}</td>
